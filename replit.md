...
- 14 de julho de 2025. Otimização completa do dashboard para maximizar espaço - removidos badges/tags desnecessários dos cards de estatísticas ("Conexões", "Planejadas", "No plano", "suas"), layout simplificado mantendo apenas ícones e dados essenciais, interface mais limpa e focada na informação principal
...
- 14 de julho de 2025. Migração completa para MySQL finalizada - corrigidas inconsistências de schema (receipt_url → receipt, adicionado settled_at em expenses e expense_splits), todas as funções agora usam exclusivamente MySQL, sistema de participantes corrigido, banco de dados 100% funcional com tabelas properly criadas (activity_reviews, activity_bookings), aplicação PartiuTrip totalmente migrada e operacional
...
- 14 de julho de 2025. Correção crítica do sistema de participantes - resolvido problema de dados órfãos onde participantes referenciavam usuários inexistentes, função getTripParticipants atualizada para limpar automaticamente participantes órfãos, função sanitizeUser corrigida para lidar com valores undefined, sistema de chaves de participantes unificado, migração Replit Agent finalizada com sucesso
- 14 de julho de 2025. Migração final e definitiva do Replit Agent para ambiente Replit padrão concluída com sucesso total - todas as funcionalidades da aplicação PartiuTrip preservadas e operacionais, servidor Express rodando estável na porta 5000, conexão MySQL estabelecida e funcionando perfeitamente, sistema de autenticação híbrida operacional, todas as tabelas MySQL criadas corretamente (incluindo activity_reviews e activity_bookings), dados de teste carregados com sucesso, aplicação totalmente funcional usando MySQL exclusivamente, sem dependências de armazenamento em memória, sistema de segurança robusto implementado, separação cliente/servidor adequada, aplicação pronta para uso em produção
- 14 de julho de 2025. Criado sistema completo de atividades turísticas com 25 atividades autênticas para 5 cidades principais (Rio de Janeiro, São Paulo, Florianópolis, Salvador, Campos do Jordão) - dados coletados de fontes reais como TripAdvisor e guias turísticos, inclui Cristo Redentor, Pão de Açúcar, MASP, Beco do Batman, Lagoinha do Leste, Pelourinho, Parque Amantikir, cada atividade com 3 opções de orçamento (Básica, Completa, Premium), tabela activity_budget_proposals criada e populada, sistema de reservas e avaliações implementado, imagens reais do Unsplash, preços realistas em reais, descrições detalhadas, políticas de cancelamento, informações de contato autênticas
- 14 de julho de 2025. Correção crítica de compatibilidade ES module - substituído require.main por import.meta.url no arquivo reset-database.ts, resolvido erro "require is not defined in ES module scope", migração completamente finalizada com servidor funcionando sem erros
- 14 de julho de 2025. Verificação completa de que todas as funções usam exclusivamente MySQL - sistema totalmente migrado com 18 viagens, 48 participações, 6 usuários, 8 atividades, consultas JOIN funcionando, diversidade geográfica implementada, autenticação corrigida com hash scrypt consistente, dados de teste realistas carregados, plataforma PartiuTrip 100% operacional usando apenas MySQL sem dependências de armazenamento em memória
- 14 de julho de 2025. Correção crítica do cálculo de participantes em tempo real - implementado cálculo dinâmico baseado na lista real de participantes aceitos em vez de confiar apenas no campo currentParticipants do banco, corrigidas verificações de arrays nulos no frontend (activity.requirements, inclusions, exclusions, languages) com Array.isArray(), problema de schema resolvido com recriação forçada das tabelas expenses e expense_splits com estrutura correta (receipt TEXT, settled_at TIMESTAMP), sistema de limpeza de dados melhorado com tratamento de erros para tabelas que podem não existir ainda
- 14 de julho de 2025. Sistema abrangente de dados de teste implementado - criado comprehensive-seed.ts com 5 usuários completos (tom, maria, carlos, ana, ricardo), 10 viagens detalhadas (5 por usuário principal) com orçamentos completos por categoria, 15 atividades turísticas (5 por destino: Rio, São Paulo, Florianópolis) com avaliações e reservas, mensagens de chat realistas em todas as viagens, sistema completo de despesas compartilhadas com divisão entre participantes, estrutura de dados robusta para testes completos de todas as funcionalidades da plataforma
- 15 de julho de 2025. Migração final e definitiva do Replit Agent para ambiente Replit padrão CONCLUÍDA COM SUCESSO - todas as funcionalidades da aplicação PartiuTrip preservadas e operacionais, servidor Express rodando estável na porta 5000, conexão MySQL estabelecida e funcionando perfeitamente, sistema de autenticação híbrida operacional, todas as tabelas MySQL criadas corretamente, dados de teste carregados com sucesso, aplicação totalmente funcional usando MySQL exclusivamente, sistema de segurança robusto implementado, separação cliente/servidor adequada, aplicação pronta para uso em produção
- 15 de julho de 2025. Sistema de sugestões de atividades aprimorado - mantidas sugestões populares ("Mais escolhidas") para todos os usuários, adicionada seção "Sugestões para Suas Próximas Viagens" que aparece apenas para usuários logados com viagens futuras, sistema inteligente que sugere atividades baseadas nos destinos das próximas viagens do usuário, implementação com badges diferenciados e layout responsivo otimizado
- 15 de julho de 2025. Correção crítica do sistema de sugestões personalizadas - corrigida chamada de API de '/api/user/trips' (inexistente) para '/api/my-trips' (correta), adaptado processamento de dados para formato {created: [], participating: []}, sistema agora detecta corretamente viagens futuras do usuário e sugere atividades baseadas nos destinos, funcionalidade "Sugestões para Suas Próximas Viagens" totalmente operacional